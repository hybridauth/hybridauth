...
abstract class OAuth2 extends AbstractAdapter implements AdapterInterface
{
    ...
    protected function initialize()
    {
        if (!$this->config->filter('keys')->get('id')) {
            throw new InvalidApplicationCredentialsException('Your application id is required in order to connect to '.
                                                             $this->providerId);
        }
    ...
    public function authenticateFinish()
    {
        ...
        if ($collection->get('error')) {
            throw new InvalidAuthorizationCodeException('Provider returned an error: '.
                                                        htmlentities($collection->get('error')));
        }
