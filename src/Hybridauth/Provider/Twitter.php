...
*/
class Hybridauth_Provider_Twitter extends Hybridauth_Core_Provider_Model_OAuth1
{
	/**
	...
	*/
	function getUserProfile()
	{
		...
		if ( $this->api->http_code != 200 ){
			throw new Hybridauth_Core_Exception( "User profile request failed! {$this->providerId} returned an error.", 6 );
		}

		if ( ! is_object( $response ) || ! isset( $response->id ) ){
			throw new Hybridauth_Core_Exception( "User profile request failed! {$this->providerId} api returned an invalid response.", 6 );
		}
	...
	*/
	function getUserContacts()
	{
		...
		// check the last HTTP status code returned
		if ( $this->api->http_code != 200 ){
			throw new Hybridauth_Core_Exception( "User contacts request failed! {$this->providerId} returned an error.");
		}
	...
	*/ 
	function setUserStatus( $status )
	{
		...
		// check the last HTTP status code returned
		if ( $this->api->http_code != 200 ){
			throw new Hybridauth_Core_Exception( "Update user status failed! {$this->providerId} returned an error.");
		}
 	}
	...
	*/ 
	function getUserActivity( $stream )
	{
		...
		// check the last HTTP status code returned
		if ( $this->api->http_code != 200 ){
			throw new Hybridauth_Core_Exception( "User activity stream request failed! {$this->providerId} returned an error.");
		}
