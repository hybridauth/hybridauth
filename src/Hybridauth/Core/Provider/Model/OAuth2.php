 ...
 */
class Hybridauth_Core_Provider_Model_OAuth2 extends Hybridauth_Core_Provider_Model
{
	// default permissions 
	public $scope = "";
	...
	*/
	function initialize() 
	{
		...
		$this->api = new Hybridauth_Core_Provider_Protocol_OAuth2( $this->config["keys"]["id"], $this->config["keys"]["secret"], $this->endpoint );
		...
		if( isset( Hybridauth_Core_Hybridauth::$config["proxy"] ) ){
			$this->api->curl_proxy = Hybridauth_Core_Hybridauth::$config["proxy"];
		}
	}
	...
	*/
	function loginBegin()
	{
		...
		Hybridauth_Core_Hybridauth::redirect( $this->api->authorizeUrl( array( "scope" => $this->scope ) ) ); 
	}
