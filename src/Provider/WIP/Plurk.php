...
class Hybrid_Providers_Plurk extends Hybrid_Provider_Model_OAuth1
{
    ...
    public function getUserProfile()
    {
        ...
        $this->user->profile->photoURL    =
            $this->getPhotoURL($profile->uid, $profile->has_profile_image, $profile->avatar);
    ...
    {
        $parameters            = [];
        $parameters['user_id'] = 0; // $this->user_id; // @fixme
        ...
        if ($this->api->http_code != 200) {
            throw new Exception('User contacts request failed! '.
                                $this->providerId.
                                ' returned an error: '.
                                $this->api->lastErrorMessageFromStatus());
        }
    ...
    public function getUserActivity($stream)
    {
        ...
        foreach ($plurks as $item) {
            ...
            $ua->user->photoURL   =
                $this->getPhotoURL($item->owner_id, $users->{$item->owner_id}->has_profile_image,
                    $users->{$item->owner_id}->avatar);
            $activities[]         = $ua;
        }
    ...
    public function getPhotoURL($user_id, $has_profile_image, $avatar)
    {
